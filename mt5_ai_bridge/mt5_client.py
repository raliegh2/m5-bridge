...
Every other module talks to a client object so the trading logic can be tested
...
class RealMT5Client:
    ...
    def __init__(self) -> None:
        ...
        self.TRADE_ACTION_PENDING = getattr(mt5, "TRADE_ACTION_PENDING", 5)
        self.TRADE_ACTION_REMOVE = getattr(mt5, "TRADE_ACTION_REMOVE", 8)
        ...
        self.TRADE_RETCODE_PLACED = getattr(
            mt5, "TRADE_RETCODE_PLACED", None
        )
        ...
        self.DEAL_ENTRY_IN = getattr(mt5, "DEAL_ENTRY_IN", 0)
        self.DEAL_ENTRY_OUT = getattr(mt5, "DEAL_ENTRY_OUT", 1)
        self.DEAL_ENTRY_INOUT = getattr(mt5, "DEAL_ENTRY_INOUT", 2)
        self.DEAL_ENTRY_OUT_BY = getattr(mt5, "DEAL_ENTRY_OUT_BY", 3)

        self.ACCOUNT_TRADE_MODE_DEMO = getattr(
            mt5, "ACCOUNT_TRADE_MODE_DEMO", 0
        )
        self.ACCOUNT_TRADE_MODE_REAL = getattr(
    ...
    def history_deals_get(self, date_from, date_to, **kwargs) -> Any:
        return self._mt5.history_deals_get(
            date_from,
            date_to,
            **kwargs,
        )
    ...
    def timeframe(self, name: str) -> Any:
        try:
            return getattr(
                self._mt5,
                f"TIMEFRAME_{name.upper()}",
            )
        except AttributeError as exc:
            raise ValueError(
                f"Unknown timeframe: {name!r}"
            ) from exc
    ...
    def copy_ticks_from(
        ...
        return self._mt5.copy_ticks_from(
            symbol,
            date_from,
            count,
            flags,
