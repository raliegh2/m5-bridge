"""Download the five official FXCM H1 bid/ask series used by V14.19."""
...
def requested_weeks(start: pd.Timestamp, end: pd.Timestamp) -> list[tuple[int, int]]:
    days = pd.date_range(
        start.floor("D") - pd.Timedelta(days=7),
        end.ceil("D"),
        freq="D",
        tz="UTC",
    )
    ...
    for day in days:
        iso = day.isocalendar()
        output.add((int(iso.year), int(iso.week)))
    ...
    for attempt in range(1, 5):
        try:
            response = requests.get(
                url,
                timeout=(15, 60),
                headers={"User-Agent": "m5-bridge-v14-19-range-shadow/1.0"},
            )
...
def identify_column(frame: pd.DataFrame, *candidates: str) -> str:
    ...
    for candidate in candidates:
        ...
        if key in names:
            return names[key]
    raise RuntimeError(
        f"Missing expected column {candidates}; available={list(frame.columns)}"
    )
...
def normalize(
        ...
        output = pd.DataFrame(
            {
                "time": merged["time"],
                "open": pd.to_numeric(
                    merged[identify_column(merged, f"{prefix}Open")],
                    errors="coerce",
                ),
                "high": pd.to_numeric(
                    merged[identify_column(merged, f"{prefix}High")],
                    errors="coerce",
                ),
                "low": pd.to_numeric(
                    merged[identify_column(merged, f"{prefix}Low")],
                    errors="coerce",
                ),
                "close": pd.to_numeric(
                    merged[identify_column(merged, f"{prefix}Close")],
                    errors="coerce",
                ),
                "tick_volume": (
    ...
    if len(bid) < MIN_BARS or len(ask) < MIN_BARS:
        raise RuntimeError(
            f"{symbol}: insufficient H1 rows bid={len(bid)} ask={len(ask)} minimum={MIN_BARS}"
        )
    quality = {
        "symbol": symbol,
    ...
    for symbol in symbols:
        ...
        with ThreadPoolExecutor(max_workers=WORKERS) as executor:
            futures = {
                executor.submit(
                    fetch,
                    BASE.format(symbol=symbol, year=year, week=week),
                ): (year, week)
        ...
        try:
            ...
            bid.to_csv(
                OUT / f"{symbol}_H1_bid.csv",
                index=False,
                date_format="%Y-%m-%dT%H:%M:%SZ",
            )
            ask.to_csv(
                OUT / f"{symbol}_H1_ask.csv",
                index=False,
                date_format="%Y-%m-%dT%H:%M:%SZ",
            )
