...
account type matches the selected DEMO/LIVE mode and every frozen V12 risk
gate passes.
"""
...
def _market_snapshot(client) -> tuple[dict, list[dict], list[dict]]:
    ...
    if account is not None:
        account_row = {
            ...
            "trade_mode": (
                "DEMO" if getattr(account, "trade_mode", None) ==
                getattr(client, "ACCOUNT_TRADE_MODE_DEMO", 0) else
                "LIVE" if getattr(account, "trade_mode", None) ==
                getattr(client, "ACCOUNT_TRADE_MODE_REAL", 2) else "OTHER"
            ),
        }
    position_rows = [
        {
...
def scanner_loop(client, adapter: FinalV12Adapter, mode_store: AccountModeStore,
                 status: SharedStatus,
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            status.update(
                ...
                selected_account_mode=mode_store.get(),
                execution_mode=f"{mode_store.get()}_AUTO",
            )
...
.modebar{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:14px}.modebtn{background:#17233d;color:#d9e5f7;border:1px solid #33476f;border-radius:8px;padding:8px 14px;cursor:pointer;font-weight:700}.modebtn.active{background:#1d5b3a;border-color:#39dda0}
...
<div class="modebar panel"><b>Account mode</b><button id="demoMode" class="modebtn" onclick="setMode('DEMO')">Demo</button><button id="liveMode" class="modebtn" onclick="setMode('LIVE')">Live</button><span id="modeMessage" class="sub">The selection must match the connected MT5 account.</span></div>
...
async function setMode(mode){let r=await fetch('/mode',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode})});let d=await r.json();document.getElementById('modeMessage').textContent=d.message||d.error||''}
...
let m=d.selected_account_mode||'DEMO',match=!a.trade_mode||a.trade_mode===m;document.getElementById('mode').textContent=m+' AUTO';document.getElementById('mode').className='badge '+(match?'green':'red');document.getElementById('demoMode').className='modebtn '+(m==='DEMO'?'active':'');document.getElementById('liveMode').className='modebtn '+(m==='LIVE'?'active':'');document.getElementById('modeMessage').textContent=match?('Connected account: '+(a.trade_mode||'checking')):('BLOCKED: selected '+m+' but connected account is '+a.trade_mode);
...
def make_handler(status: SharedStatus, mode_store: AccountModeStore):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            try:
                ...
                mode = mode_store.set(payload.get("mode", ""))
                status.update(selected_account_mode=mode, execution_mode=f"{mode}_AUTO")
                body = json.dumps({
                    "ok": True, "mode": mode,
                    "message": f"Automatic execution mode switched to {mode}.",
                }).encode()
                code = 200
...
def choose_startup_mode(mode_store: AccountModeStore) -> str:
    current = mode_store.get()
    while True:
        try:
            answer = input(
                f"Use this bot on a DEMO or LIVE account? [{current}]: "
            ).strip().upper()
        except EOFError:
            answer = current
        if not answer:
            answer = current
        try:
            return mode_store.set(answer)
        except ValueError:
            print("Please enter DEMO or LIVE.")


def terminal_mode_loop(mode_store: AccountModeStore, status: SharedStatus,
                       stop_event: threading.Event) -> None:
    print("Runtime command: type MODE DEMO or MODE LIVE, then press Enter.")
    while not stop_event.is_set():
        try:
        ...
        if len(parts) == 2 and parts[0] == "MODE":
            try:
                mode = mode_store.set(parts[1])
                status.update(selected_account_mode=mode,
                              execution_mode=f"{mode}_AUTO")
                print(f"Automatic account mode switched to {mode}.")
            except ValueError as exc:
                print(exc)
        else:
            print("Unknown command. Use MODE DEMO or MODE LIVE.")
...
def main() -> None:
    ...
    parser.add_argument("--account-mode", choices=("DEMO", "LIVE"))
    parser.add_argument("--no-mode-prompt", action="store_true")
    ...
    if args.account_mode:
        mode_store.set(args.account_mode)
    elif not args.no_mode_prompt:
        choose_startup_mode(mode_store)
    client = create_client()
    connect(client, settings)
    status = SharedStatus()
    status.update(engines=_engine_rows(), selected_account_mode=mode_store.get(),
                  execution_mode=f"{mode_store.get()}_AUTO")
    ...
    worker = threading.Thread(
        target=scanner_loop,
        args=(client, adapter, mode_store, status, args.interval,
              args.lookback_hours, stop_event),
        daemon=True,
    )
    worker.start()
    command_worker = threading.Thread(
        target=terminal_mode_loop, args=(mode_store, status, stop_event), daemon=True)
    command_worker.start()
    server = ThreadingHTTPServer(
        ("127.0.0.1", args.port), make_handler(status, mode_store))
    url = f"http://127.0.0.1:{args.port}"
    print(f"V12 dashboard: {url}")
    print("Dashboard refresh: 1 second")
    print("Strategy scan interval: 1 second")
    print(f"Execution mode: automatic {mode_store.get()} MT5 order submission")
