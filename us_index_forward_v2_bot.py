"""Automatic demo forward-test runner for US_INDEX_FORWARD_V2.

Loads the frozen V2 artifact and its passed historical report. The signal feed can
...
from mt5_ai_bridge.us_index_forward_v2 import LOCKED_CONFIG, latest_decision, load_artifact

ROOT = Path(__file__).resolve().parent
MODEL_PATH = ROOT / "research" / "us_index_forward_v2_model.json"
RESULT_PATH = ROOT / "research" / "us_index_forward_v2_result.json"
STATE_PATH = ROOT / "state" / "us_index_forward_v2_state.json"
MAGIC = 20260825
...
def _broker_lots(info, balance: float, price: float, stop_distance: float,
    ...
    elif _truthy("US_INDEX_FORWARD_V2_APPLY_NOTIONAL_CAP", "false"):
        contract = float(getattr(info, "trade_contract_size", 0.0) or 0.0)
        if contract > 0 and price > 0:
            raw = min(raw, balance * LOCKED_CONFIG.max_fraction_invested * multiplier / (price * contract))
...
def main(argv=None) -> int:
    ...
    setup_logging()
    if not _truthy("US_INDEX_FORWARD_V2_ENABLED"):
        print("US_INDEX_FORWARD_V2_ENABLED is false. Nothing to do.")
        return 0
    if not MODEL_PATH.exists() or not RESULT_PATH.exists():
        print("Missing V2 model/result; run research/train_us_index_forward_v2.py first.")
        return 2

    artifact = load_artifact(MODEL_PATH)
    result = _load_json(RESULT_PATH)
    if not bool(result.get("forward_test_ready")):
        print("V2 stronger-model gate did not pass; auto forward testing is blocked.")
    ...
    signal_symbol = os.getenv("US_INDEX_FORWARD_V2_SIGNAL_SYMBOL", LOCKED_CONFIG.research_symbol).strip()
    execution_symbol = os.getenv("US_INDEX_FORWARD_V2_EXEC_SYMBOL", signal_symbol).strip()
    ...
    try:
        ...
        if settings.mode is Mode.AUTO and int(getattr(account, "trade_mode", -1)) != 0:
            print("AUTO blocked: US_INDEX_FORWARD_V2 requires an explicit DEMO account.")
            return 4
        ...
        if client.symbol_info(signal_symbol) is None or info is None:
            print("Signal or execution symbol unavailable.")
            return 2
        ...
        if rates is None or len(rates) < 90:
            print("Not enough completed D1 signal bars for V2.")
            return 2
        ...
        if not args.force and int(state.get("last_processed_bar", 0) or 0) == decision.time:
            print("Latest completed V2 signal bar already processed.")
            return 0
        ...
        if total_dd >= 0.20 or daily_loss >= 0.02:
            print(f"Risk stop: total_dd={total_dd:.2%}, daily_loss={daily_loss:.2%}")
            _save_state(state)
            return 5
        ...
        for p in list(positions):
            ...
            if opposite or timed_out:
                reason = "opposite V2 signal" if opposite else "maximum V2 holding period"
        ...
        # Scale signal ATR into execution-price units for close proxies/futures mappings.
        ...
        if lots <= 0:
            print("V2 risk-sized volume is below broker minimum/exposure cap; no trade.")
            state["last_processed_bar"] = decision.time
            _save_state(state)
            return 0
