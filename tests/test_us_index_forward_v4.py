...
from mt5_ai_bridge.us_index_forward_v4 import CANDIDATES, LOCKED_CONFIG, fit_model
...
def _bars() -> pd.DataFrame:
    ...
    seconds = np.asarray([int(ts.timestamp()) for ts in dates], dtype=np.int64)
    return pd.DataFrame({
        "time": seconds,
        "open": open_,
        "high": high,
...
def test_v4_is_h4_and_keeps_one_percent_ceiling():
    assert LOCKED_CONFIG.timeframe == "H4"
    assert LOCKED_CONFIG.risk_percent == 1.0
    assert LOCKED_CONFIG.fast_horizon == 2
    assert LOCKED_CONFIG.slow_horizon == 6
    assert min(c.signal_quantile for c in CANDIDATES) <= 0.54
    assert max(c.max_holding_bars for c in CANDIDATES) <= 10


def test_v4_training_requires_five_years():
    bars = _bars()
    ...
    try:
        ...
        raise AssertionError("V4 should reject less than five years of training")


def test_v4_post_cutoff_mutation_cannot_change_selection_or_fit():
    bars = _bars()
    ...
    factor = np.linspace(0.30, 2.60, int(mask.sum()))
