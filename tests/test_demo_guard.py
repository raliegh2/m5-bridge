"""AUTO fails closed unless MT5 explicitly reports a demo account."""

from types import SimpleNamespace

from mt5_ai_bridge.app import (
    _account_kind,
    _is_demo_account,
    _is_real_account,
    run,
...
def test_account_kind_and_demo_detection():
    ...
    assert _account_kind(make_account(trade_mode=1)) == "CONTEST"
    ...
    assert _is_real_account(make_account(trade_mode=2)) is True
    assert _is_real_account(make_account(trade_mode=0)) is False
...
def test_auto_real_account_is_blocked(tmp_path):
    ...
    run(
        settings=make_settings(mode=Mode.AUTO, require_demo=True, db_path=db),
        client=client,
        journal=Journal(db),
        strategy_fn=_buy,
        max_iterations=1,
...
def test_require_demo_false_allows_explicit_real_override(tmp_path):
    client = _client(2)
    db = str(tmp_path / "override.db")
    run(
        settings=make_settings(mode=Mode.AUTO, require_demo=False, db_path=db),
        client=client,
        journal=Journal(db),
        strategy_fn=_buy,
        max_iterations=1,
